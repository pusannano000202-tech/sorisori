...
fn start_sidecars(
    ...
    if should_spawn_sidecar(8787, "realtime", log) {
        match sidecar_command(&realtime_exe)
            ...
            .env("LOCAL_AI_BRIDGE_SILENCE_RMS_THRESHOLD", "60")
            .env("LOCAL_AI_BRIDGE_SILENCE_CHUNKS_REQUIRED", "20")
            .env("LOCAL_AI_BRIDGE_MIN_SPEECH_CHUNKS", "32")
            .env("LOCAL_AI_BRIDGE_MAX_SPEECH_CHUNKS", "180")
            .stdout(Stdio::null())
            .stderr(Stdio::piped())
            .spawn()
        {
